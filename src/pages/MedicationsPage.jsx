...
export default function MedicationsPage() {
  ...
  async function updateMedication(id, patch) {
    ...
    if (error) {
      ...
      alert(t('medications.save_error', 'Errore durante il salvataggio. Riprova.'))
    }
  }
  ...
  async function deleteMedication(id) {
    ...
    if (error) {
      setMeds(previous)
      scheduleMedicationReminders(previous.filter(m => m.active))
      alert(t('medications.delete_error', 'Errore durante l\'eliminazione. Riprova.'))
    }
  }
  ...
  return (
    <div className="page">
      <div style={{ background: 'linear-gradient(160deg, #7c2d12, #c2410c)', padding: 'calc(env(safe-area-inset-top) + 20px) 20px 28px' }}>
        <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 11, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '.05em' }}>{t('medications.header_eyebrow', 'Salute')}</p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <h1 style={{ fontFamily: 'var(--font-d)', fontSize: 22, color: 'white', fontWeight: 300 }}>{t('medications.header_title', 'Farmaci e Integratori')}</h1>
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 12 }}>{t('medications.header_subtitle', 'Promemoria per ogni assunzione')}</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '16px 16px 120px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {!permGranted && !notSupported && (
          <div style={{ background: '#fff4e6', border: '1.5px solid #f0922b', borderRadius: 14, padding: '14px 16px' }}>
            <p style={{ fontSize: 13, color: '#b45309', lineHeight: 1.6, marginBottom: 10 }}>
              {t('medications.enable_notifications_text', "Attiva le notifiche per ricevere i promemoria all'orario giusto.")}
            </p>
            <button onClick={handleEnableNotifications} className="btn btn-primary" style={{ fontSize: 13, padding: '9px 16px' }}>
              {t('medications.enable_notifications_btn', 'Attiva notifiche')}
            </button>
          </div>
        )}
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="card" style={{ padding: 20 }}>
          ...
          <button onClick={addTimeSlot} style={{ background: 'none', border: 'none', color: '#c2410c', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, padding: '4px 0', marginBottom: 14 }}>
            <Plus size={14} /> {t('medications.add_time_btn', 'Aggiungi orario')}
          </button>
          ...
          >
            <Plus size={16} /> {saving ? t('medications.saving', 'Salvataggio…') : t('medications.add_medication', 'Aggiungi farmaco')}
          </button>
        </motion.div>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div style={{ padding: '28px 16px', textAlign: 'center', background: 'var(--surface)', borderRadius: 14, border: '1px solid var(--border-light)' }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>{t('medications.empty_title', 'Nessun farmaco impostato')}</p>
              <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>{t('medications.empty_subtitle', 'Aggiungine uno sopra per ricevere i promemoria')}</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {meds.map(med => (
                <div key={med.id} className="card" style={{ padding: 16, opacity: med.active ? 1 : 0.55 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: med.times?.length ? 10 : 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      ...
                      <button onClick={() => deleteMedication(med.id)} aria-label={t('medications.delete_aria', 'Elimina farmaco')} style={{ background: '#fff0f0', border: 'none', borderRadius: 8, width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--red)', cursor: 'pointer' }}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
                  ...
                  <button onClick={() => addMedTime(med)} style={{ background: 'none', border: 'none', color: '#c2410c', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, padding: '2px 0' }}>
                    <Plus size={12} /> {t('medications.add_time_btn', 'Aggiungi orario')}
                  </button>
                </div>
              ))}
            </div>
