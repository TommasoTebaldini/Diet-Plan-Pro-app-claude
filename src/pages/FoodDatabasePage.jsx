    ...
    <div>
      {pending ? (
        ...
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          <input type="text" className="input-field" placeholder={t('food.ingredient_search')} value={q} onChange={e => setQ(e.target.value)} style={{ flex: 1 }} autoComplete="off" />
...
export default function FoodDatabasePage() {
  ...
  async function saveRicetta() {
    ...
    const totals = mealTotals(recForm.ingredienti)
    ...
    const { data } = await supabase.from('ricette').insert({
      ...
      peso_totale_g: pesoTotale,
      kcal_100g: kcal100,
      proteins_100g: pesoTotale > 0 ? Math.round(totals.proteins / pesoTotale * 1000) / 10 : 0,
      carbs_100g: pesoTotale > 0 ? Math.round(totals.carbs / pesoTotale * 1000) / 10 : 0,
      fats_100g: pesoTotale > 0 ? Math.round(totals.fats / pesoTotale * 1000) / 10 : 0,
  ...
  const TABS = [
    ['search', t('food.tab_search')],
    ['saved', t('food.tab_saved')],
    ['meals', t('food.tab_meals')],
    ['ricette', t('food.tab_recipes')],
  ]
        ...
        {tab === 'search' && (
          <>
            <form onSubmit={handleSearch} style={{ display: 'flex', gap: 8 }}>
              <input type="text" className="input-field" placeholder={t('food.search_placeholder')} value={query} onChange={e => setQuery(e.target.value)} style={{ flex: 1 }} autoComplete="off" />
        ...
        {tab === 'saved' && (
          <>
            <button className="btn btn-primary" onClick={() => setShowAdd(v => !v)} style={{ alignSelf: 'flex-start' }}>
              {showAdd ? <X size={15} /> : <Plus size={15} />}{showAdd ? t('common.cancel') : t('food.add_manual')}
            </button>

            {showAdd && (
              <div className="card animate-slideUp" style={{ padding: 14 }}>
                <p style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>{t('food.new_food')}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    ...
                    <div className="input-group">
                      ...
                      <input className="input-field" value={form.brand} onChange={set('brand')} placeholder={t('food.optional')} />
                    </div>
                    <div className="input-group">
                      <label className="input-label">{t('food.kcal_100g')}</label>
                      <input type="number" className="input-field" value={form.kcal_100g} onChange={set('kcal_100g')} placeholder="0" inputMode="decimal" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">{t('food.proteins_100g')}</label>
            ...
            {recentFoods.length > 0 && (
              <div className="card" style={{ padding: '12px 14px' }}>
                <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 10 }}>{t('food.frequent')}</p>
            ...
            {saved.length === 0 && !showAdd && recentFoods.length === 0 && (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontSize: 14, fontWeight: 500 }}>{t('food.no_favorites')}</p>
                <p style={{ fontSize: 12, marginTop: 4 }}>{t('food.no_favorites_desc')}</p>
              </div>
            )}

            {saved.length > 0 && <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)' }}>{t('food.saved_label')}</p>}
            {saved.map(f => (
        ...
        {tab === 'meals' && (
          <>
            <button className="btn btn-primary" onClick={() => setShowMealCreate(v => !v)} style={{ alignSelf: 'flex-start' }}>
              {showMealCreate ? <X size={15} /> : <Plus size={15} />}{showMealCreate ? t('common.cancel') : t('food.create_meal')}
            </button>
            ...
            {meals.length === 0 && !showMealCreate && (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontSize: 14, fontWeight: 500 }}>{t('food.no_meals')}</p>
                <p style={{ fontSize: 12, marginTop: 4 }}>{t('food.no_meals_desc')}</p>
              </div>
            )}
        ...
        {tab === 'ricette' && (
          <>
            <button className="btn btn-primary" onClick={() => setShowRecCreate(v => !v)} style={{ alignSelf: 'flex-start' }}>
              {showRecCreate ? <X size={15} /> : <Plus size={15} />}{showRecCreate ? t('common.cancel') : t('food.import_recipe')}
            </button>

            {showRecCreate && (
              <div className="card animate-slideUp" style={{ padding: 14 }}>
                <p style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>{t('food.new_recipe')}</p>
                ...
                <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 8 }}>{t('food.ingredients_label')}</p>
                ...
                {recForm.ingredienti.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    ...
                    {(() => {
                      const mt = mealTotals(recForm.ingredienti)
            ...
            {ricette.length === 0 && !showRecCreate && (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontSize: 14, fontWeight: 500 }}>{t('food.no_recipes')}</p>
                <p style={{ fontSize: 12, marginTop: 4 }}>{t('food.no_recipes_desc')}</p>
              </div>
            )}
                  ...
                  {isOpen && (
                    <div style={{ borderTop: '1px solid var(--border-light)', padding: '10px 14px 12px' }}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                        ...
                        <span style={{ fontSize: 11, background: '#f1f5f9', padding: '2px 7px', borderRadius: 100, color: 'var(--text-muted)' }}>{t('food.per_portion')}</span>
                      </div>
