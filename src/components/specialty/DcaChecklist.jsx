  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        <HeartPulse size={16} color="#991B1B" />
        <h3 style={{ fontSize: 14, fontWeight: 700 }}>I tuoi pasti di oggi</h3>
      </div>
        ...
        {pasti.map((m, i) => {
          ...
          return (
            <button key={i} onClick={() => toggle(key)} style={{
              ...
              background: done ? '#FEE2E2' : 'var(--surface)', cursor: 'pointer', font: 'inherit', textAlign: 'left',
            }}>
              <div style={{
                width: 20, height: 20, borderRadius: 6, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
                border: `1.5px solid ${done ? '#991B1B' : 'var(--border)'}`,
                background: done ? '#991B1B' : 'transparent',
              }}>
      ...
      {doneCount > 0 && (
        <p style={{ fontSize: 12, color: '#991B1B', marginTop: 10, textAlign: 'center' }}>
