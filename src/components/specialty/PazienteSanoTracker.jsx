...
export default function PazienteSanoTracker({ dati }) {
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 14 }}>{t('pazsano.title', '🌿 Il tuo fabbisogno')}</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${[bmr, tdee, kcalTarget].filter(v => v !== null).length}, 1fr)`, gap: 8, marginBottom: kcalTarget ? 16 : 0 }}>
        {bmr !== null && (
          <div style={{ textAlign: 'center', padding: '10px 6px', background: 'var(--surface-2)', borderRadius: 10 }}>
            <p style={{ fontSize: 10, color: 'var(--text-muted)' }}>{t('pazsano.bmr', 'Metabolismo basale')}</p>
            <p style={{ fontSize: 16, fontWeight: 700 }}>{bmr}</p>
          </div>
        )}
        {tdee !== null && (
          <div style={{ textAlign: 'center', padding: '10px 6px', background: 'var(--surface-2)', borderRadius: 10 }}>
            <p style={{ fontSize: 10, color: 'var(--text-muted)' }}>{t('pazsano.tdee', 'Fabbisogno totale')}</p>
            <p style={{ fontSize: 16, fontWeight: 700 }}>{tdee}</p>
          </div>
        )}
        {kcalTarget !== null && (
          <div style={{ textAlign: 'center', padding: '10px 6px', background: '#F0FDF4', borderRadius: 10 }}>
            <p style={{ fontSize: 10, color: 'var(--text-muted)' }}>{t('pazsano.dailyTarget', 'Target giornaliero')}</p>
      ...
      {kcalTarget !== null && intake !== null && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 5 }}>
            <span style={{ fontWeight: 500 }}>{t('pazsano.consumedToday', 'Assunte oggi')}</span>
            <span><b>{Math.round(intake.kcal)}</b> / {kcalTarget} {t('pazsano.kcalUnit', 'kcal')}</span>
          </div>
      ...
      {intake !== null && (intake.proteins > 0 || intake.carbs > 0 || intake.fats > 0) && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 14 }}>
          <div style={{ textAlign: 'center', padding: '8px 4px', background: 'var(--surface-2)', borderRadius: 10 }}>
            ...
            <p style={{ fontSize: 9.5, color: 'var(--text-muted)' }}>{t('pazsano.proteins', 'Proteine')}</p>
          </div>
          <div style={{ textAlign: 'center', padding: '8px 4px', background: 'var(--surface-2)', borderRadius: 10 }}>
            <p style={{ fontSize: 14, fontWeight: 700 }}>{Math.round(intake.carbs)}g</p>
            <p style={{ fontSize: 9.5, color: 'var(--text-muted)' }}>{t('pazsano.carbs', 'Carboidrati')}</p>
          </div>
          <div style={{ textAlign: 'center', padding: '8px 4px', background: 'var(--surface-2)', borderRadius: 10 }}>
            <p style={{ fontSize: 14, fontWeight: 700 }}>{Math.round(intake.fats)}g</p>
            <p style={{ fontSize: 9.5, color: 'var(--text-muted)' }}>{t('pazsano.fats', 'Grassi')}</p>
          </div>
        </div>
      )}
    </div>
