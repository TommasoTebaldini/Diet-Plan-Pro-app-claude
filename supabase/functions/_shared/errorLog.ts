...
// email via Resend con dedup di un'ora. Speculare a
// NutriPlan-Pro/supabase/functions/_shared/errorLog.ts (stesso progetto
// Supabase condiviso). Vedi api/_errorLog.js per il contesto completo sul
// perché esiste (era il gap più concreto del logger in-house: nessuna
// copertura server-side, nessun alert).
...
export async function logServerError(fnName: string, err: unknown) {
  ...
  try {
    ...
    const { data: existing } = await sb.from('client_errors').select('id')
      .eq('app', 'diet-plan-pro-server').eq('page_url', fnName).eq('message', message)
