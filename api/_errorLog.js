...
// (Vercel) sulla stessa tabella `client_errors` già usata da src/lib/errorLogger.js
// (vedi commento lì: "sostituto leggero di un servizio esterno tipo Sentry,
// nessun costo/account terzo"). Qui estendiamo la stessa filosofia al lato
// server, che finora non scriveva da nessuna parte in caso di errore — vedi
// Analisi Scalabilità: una Edge Function rimasta rotta per settimane senza
// che nessuno se ne accorgesse è esattamente il tipo di buco che questo file
// chiude. Equivalente 1:1 di NutriPlan-Pro/api/_errorLog.js (stesso progetto
// Supabase condiviso, tabella client_errors in comune).
//
// In più, a differenza del logger client, invia un alert email best-effort
// via Resend la prima volta che un dato errore compare in un'ora — non ad
// ogni occorrenza, per non floodare la casella su un errore ripetuto in loop.
//
// Env richieste: SUPABASE_URL, SUPABASE_ANON_KEY (insert pubblico via policy
...
const SUPABASE_URL = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || 'https://hvdwqowkhutfsdpiubxe.supabase.co';
...
// Va sempre chiamata con `.catch(() => {})` dal chiamante: un logger di
// errori non deve mai generare altri errori né rallentare/bloccare la
// risposta all'utente.
export async function logServerError(fnName, err, req) {
  // Fallback a VITE_SUPABASE_ANON_KEY: vedi commento in api/food-swap.js —
  // questo progetto Vercel ha solo la variabile prefissata per il bundle client.
  ...
  try {
    ...
    const qs = `select=id&app=eq.diet-plan-pro-server&page_url=eq.${encodeURIComponent(fnName)}&message=eq.${encodeURIComponent(message)}&created_at=gte.${since}&limit=1`;
...
// rejection non gestita esplicitamente all'interno e rispondere comunque 500
// invece di far fallire la funzione in modo silenzioso. Non sostituisce i
// try/catch già presenti nei singoli handler — quelli vanno comunque
// istruiti a chiamare logServerError prima di rispondere, questa è solo la
// rete di sicurezza per ciò che sfugge.
